              ...
              child: Material(
                  ...
                  child: TextField(
                      ...
                      decoration: InputDecoration(
                          border: InputBorder.none,
                          contentPadding: const EdgeInsets.all(5),
                          prefixIcon: SvgPicture.asset('assets/vectors/at.svg',
                              fit: BoxFit.scaleDown),
...
class _EnterPhoneState extends State<EnterPhone> {
  ...
  checkUsername() async {
    ...
    if (finalPhone == '' || finalPhone.isEmpty) {
      setState(() {
          ...
          Form(
              ...
              child: Column(children: <Widget>[
                IntlPhoneField(
                    ...
                    onChanged: (phone) {
                      debugPrint('Select country: ${phone.completeNumber}');
                      setState(() {
                        finalCountry = phone.countryCode;
                        finalPhone = phone.completeNumber;
                      });
                    }),
                const SizedBox(height: 20),
                Row(children: [
...
class _VerifyCodeState extends State<VerifyCode> with WidgetsBindingObserver {
  ...
  checkCode() async {
    ...
    if (code == '') {
      ...
      var responseData = response.data;
      if (responseData['message'].toString() == 'OTP Verified') {
        var snackBar = const SnackBar(
            content:
